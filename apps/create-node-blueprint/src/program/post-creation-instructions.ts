...
export function showPostCreationInstructions(options: PostCreationInstructionsOptions): void {
	...
	const dbCommands = {
		[OrmEnum.prisma]: ["Generate migration: npx prisma migrate dev", "Apply migration: npx prisma migrate deploy"],
		[OrmEnum.drizzle]: ["Generate migration: npm run db:generate", "Apply migration: npm run db:migrate"],
		[OrmEnum.mongoose]: ["No migrations needed for MongoDB"],
	}[orm];
